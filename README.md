...
The hard part isn't fetching prices; it's deciding that two differently worded listings are the same phone, using rules general enough to work on a model that doesn't exist yet, with no hardcoded product list.
...
Each scraper re-scrapes every 900 seconds (`SCRAPE_INTERVAL_SEC` in `docker-compose.yml`), independently per retailer: the three containers don't wait on each other, so the catalog is always a merge of three snapshots taken at slightly different times.
...
`.env.example` walks through getting the Ripley cookie from a browser's dev tools (it expires in ~30 minutes). Without it, the portal still runs and shows "Ripley: no data" while comparing the other two.
...
- **Go for the unifier, TypeScript for the scrapers.** The scrapers need a real browser (Playwright) to get past Paris's WAF and Ripley's Cloudflare challenge: Node's ecosystem for that is stronger. The unifier is a tight, testable transform over a few hundred JSON rows, where Go's static typing and single-binary deploy fit better than adding a second Node process (`unifier/main.go`, `scrapers/src/browser.ts`).
- **Cookie-based bypass instead of a paid unlocker.** `RIPLEY_CF_COOKIE` and `SCRAPER_UA` (`docker-compose.yml`, `.env.example`) are a manually obtained `cf_clearance` cookie tied to the browser that got it. It is explicitly a demo-grade workaround, not a resilient integration: documented rather than hidden.
